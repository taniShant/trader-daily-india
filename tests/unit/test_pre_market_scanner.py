...
def test_scan_stocks_sorts_by_watchlist_score_and_stores_reasoned_watchlist(monkeypatch):
    scanner = make_scanner(watchlist_size=2, min_avg_volume=1000)
    scanner.get_nifty_stocks = lambda: ["RELIANCE", "TCS", "INFY"]
    ...
    assert [item["symbol"] for item in watchlist] == ["RELIANCE", "INFY"]
