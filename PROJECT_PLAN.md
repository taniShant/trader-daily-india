...
| Priority | ID | Work Package | Status | Primary Files | Required Tests | Definition Of Done |
|---|---|---|---|---|---|---|
| P1 | P13-WP01 | Add setup-level expectancy reporting | Implemented | `agent/main.py`, `agent/storage/repositories.py`, `containers/dashboard/api_server.py`, tests | Focused tests cover `/api/micro/expectancy` grouping by setup, exit reason, realized R, holding time, gross/net P&L, and costs. User VS Code verification pending. | We can prove which setup families make or lose paper money before tuning thresholds. |
| P1 | P13-WP02 | Add early invalidation exit rules | Implemented | `agent/micro/models.py`, `agent/micro/engine.py`, `agent/main.py`, `tests/unit/test_micro_trading.py`, `tests/unit/test_micro_trading_runtime.py` | Tests cover momentum-fade early exit for a held long micro position. User VS Code verification pending. | Bad micro entries are cut before the full timeout when the original reason for the trade is no longer valid. |
| P1 | P13-WP03 | Tune target/stop/timeout by setup type | Implemented | `agent/micro/models.py`, `agent/micro/setups.py`, `agent/config.py`, `agent/signals/technical.py`, `cicd/env/prod.json`, `cicd/cdk/stacks/agent_runtime_stack.py`, tests | Tests cover continuation bracket, timeout, strict two-candle continuation confirmation, and first-candle spike rejection. CDK synth exposes confirmation and VWAP-extension knobs. User VS Code verification pending. | Continuation, VWAP momentum, and opening-range setups use brackets that match their observed movement profile; volume-continuation entries require second-candle confirmation and controlled VWAP extension. |
...
| P2 | P13-WP05 | Add symbol/setup throttling from recent losses | Implemented | `agent/main.py`, `agent/micro/engine.py`, `agent/storage/repositories.py`, tests | Tests cover per-symbol throttling after repeated recent losses and setup-level throttling after repeated negative-expectancy exits. User VS Code verification pending. | The bot reduces exposure to symbols and setup families that are losing in the current session instead of repeating the same weak edge. |
...
| P1 | P13-WP09 | Block fake continuation spikes after paper-loss review | Implemented | `agent/micro/models.py`, `agent/micro/setups.py`, `agent/micro/engine.py`, `agent/config.py`, `agent/main.py`, `cicd/env/prod.json`, `cicd/cdk/stacks/agent_runtime_stack.py`, `.env.example`, `tests/.env.example`, `tests/unit/test_micro_trading.py`, `tests/unit/test_config.py` | Local: `python -m pytest tests/unit/test_micro_trading.py tests/unit/test_micro_trading_runtime.py tests/unit/test_config.py tests/unit/test_cdk_synth_script.py tests/unit/test_ecs_trading_topology.py -q` -> 52 passed; `bash scripts/verify_cdk_synth.sh` -> passed; py_compile/json/diff checks passed. User VS Code verification pending. | The bot no longer takes first-candle exceptional continuation entries, caps continuation VWAP extension at 2 ATR, uses 1 bps slippage per the updated assumption, and stops a setup family after 2 same-day losing exits with negative expectancy. |
| P1 | P13-WP10 | Add VWAP pullback continuation entry style | Implemented | `agent/micro/models.py`, `agent/micro/setups.py`, `agent/config.py`, `agent/main.py`, `cicd/env/prod.json`, `cicd/cdk/stacks/agent_runtime_stack.py`, `.env.example`, `tests/.env.example`, `tests/unit/test_micro_trading.py`, `tests/unit/test_config.py` | Local: `make verify` -> 339 passed, 2 warnings; CDK synth passed and shows `Micro VWAP Pullback`. User VS Code verification pending. | Adds a second micro entry style for stocks that spike away from VWAP, pull back near VWAP, and then resume. This increases opportunity without relaxing the no-chasing controls on overextended first-spike continuation entries. |
...
```text
Date: 2026-08-14
Work Package: Phase 13 - Micro Strategy Edge And Profit Quality
Status: Implemented
Files Changed: agent/config.py, agent/main.py, agent/micro/models.py, agent/micro/setups.py, agent/micro/engine.py, agent/storage/repositories.py, containers/dashboard/api_server.py, cicd/env/prod.json, cicd/cdk/stacks/agent_runtime_stack.py, .env.example, tests/.env.example, tests/unit/test_micro_trading.py, tests/unit/test_micro_trading_runtime.py, tests/unit/test_config.py, tests/integration/test_dashboard_api.py, PROJECT_PLAN.md
What Changed: Added setup-specific micro brackets and timeouts, early invalidation exits, per-symbol recent-loss throttling, prior-cycle symbol health ranking, cost/slippage-aware paper P&L, in-memory expectancy snapshots, and `/api/micro/expectancy` for setup-level paper-trade analysis. Newly persisted micro exit rows now carry gross P&L, costs, net P&L, setup, exit reason, expected R, realized R, holding time, and entry-quality metadata. Runtime daily P&L and consecutive-loss counters now update from net realized exits.
...
```text
Date: 2026-08-17
Work Package: P13-WP03 - Second-candle continuation confirmation
Status: Implemented
Files Changed: agent/signals/technical.py, agent/micro/models.py, agent/micro/setups.py, agent/config.py, agent/main.py, cicd/env/prod.json, cicd/cdk/stacks/agent_runtime_stack.py, .env.example, tests/unit/test_micro_trading.py, tests/unit/test_config.py, PROJECT_PLAN.md, IMPORTANTFINDINGS.md
What Changed: Added candle-direction features and a confirmation gate for `micro_volume_continuation`. Continuation now requires the latest and previous 1-minute candles to confirm in the trade direction; the later P13-WP09 paper-loss fix removed the exceptional first-candle shortcut entirely.
...
```text
Date: 2026-08-19
Work Package: P13-WP09 - Block fake continuation spikes after paper-loss review
Status: Implemented
Files Changed: agent/config.py, agent/main.py, agent/micro/models.py, agent/micro/setups.py, agent/micro/engine.py, cicd/env/prod.json, cicd/cdk/stacks/agent_runtime_stack.py, .env.example, tests/.env.example, tests/unit/test_micro_trading.py, tests/unit/test_config.py, PROJECT_PLAN.md, IMPORTANTFINDINGS.md
What Changed: Tightened the loss-making `micro_volume_continuation` path after reviewing 5-day paper logs. Removed the exceptional first-candle continuation shortcut, reduced max continuation VWAP extension to 2 ATR, added setup-level loss throttling from same-day expectancy, exposed the new controls through typed config/CDK/ECS env, and changed production paper slippage from 2 bps to 1 bps.
Test Command: python -m py_compile agent/main.py agent/micro/engine.py agent/micro/setups.py agent/micro/models.py agent/config.py cicd/cdk/stacks/agent_runtime_stack.py; python -m pytest tests/unit/test_micro_trading.py tests/unit/test_micro_trading_runtime.py tests/unit/test_config.py tests/unit/test_cdk_synth_script.py tests/unit/test_ecs_trading_topology.py -q; python -m json.tool cicd/env/prod.json; bash scripts/verify_cdk_synth.sh; git diff --check
Test Result: py_compile passed. Focused config/micro/runtime/CDK tests passed with 52 tests. JSON and diff checks passed. CDK synth passed and prints `Micro Continuation Max VWAP Extension: 2 ATR` and `Micro Setup Loss Throttle: 2 losses after 2 trades`.
Notes / Next Step: Rebuild/push and redeploy the trading-bot image. In ECS logs, confirm the startup config shows the new VWAP extension and setup loss throttle values. Paper-run expectation: after two same-day losing exits from `micro_volume_continuation`, later continuation candidates should log `setup_loss_throttle_active`.
```

```text
Date: 2026-08-20
Work Package: P13-WP10 - VWAP pullback continuation entry style
Status: Implemented
Files Changed: agent/config.py, agent/main.py, agent/micro/models.py, agent/micro/setups.py, cicd/env/prod.json, cicd/cdk/stacks/agent_runtime_stack.py, .env.example, tests/.env.example, tests/unit/test_micro_trading.py, tests/unit/test_config.py, PROJECT_PLAN.md, IMPORTANTFINDINGS.md
What Changed: Added `micro_vwap_pullback_continuation` as a deterministic micro setup. It looks for a prior impulse that stretched at least 2 ATR away from VWAP, a pullback close near VWAP, and a latest 1-minute candle that resumes in the original direction with sufficient relative volume. The setup uses the existing VWAP target/stop/hold bracket and the same risk, cost, live-quote, and paper/live broker path as other micro entries.
Test Command: `make verify`
Test Result: Passed locally. Full pytest: 339 passed, 2 existing UTC deprecation warnings. CDK synth shows `Micro VWAP Pullback: impulse>=2 ATR, entry_ext<=1.1 ATR, rv>=1.2x`.
Notes / Next Step: Rebuild/push and redeploy the trading-bot image. In ECS logs, confirm startup prints `Micro VWAP Pullback` and watch for `micro_vwap_pullback_continuation` in actionable details/trade events.
```
