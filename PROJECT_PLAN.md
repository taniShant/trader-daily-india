...
| P9-WP03 | Fix ECS image deploy workflow | Implemented | `.github/workflows/deploy.yml`, `cicd/stacks/iam_stack.py`, `cicd/env/prod.json`, `scripts/verify_deploy_path.py`, `tests/unit/test_deploy_workflow.py`, `tests/unit/test_github_oidc_role.py` | Local: deploy/OIDC workflow tests passed and `make verify` passed with 181 tests plus CDK synth. GitHub dry-run/manual deploy pending after deploying IAM OIDC role. | Builds trading bot and dashboard images with `latest` and immutable SHA tags, assumes a repo-scoped GitHub OIDC role, deploys CDK, then forces ECS service refresh and waits for stability. |
