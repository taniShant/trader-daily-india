...
| P1 | P13-WP03 | Tune target/stop/timeout by setup type | Implemented | `agent/micro/models.py`, `agent/micro/setups.py`, `agent/config.py`, `cicd/env/prod.json`, `cicd/cdk/stacks/agent_runtime_stack.py`, tests | Tests cover continuation bracket and timeout; config/CDK synth expose continuation, VWAP, and opening-range bracket knobs. User VS Code verification pending. | Continuation, VWAP momentum, and opening-range setups use brackets that match their observed movement profile. |
