...
| P1 | P13-WP03 | Tune target/stop/timeout by setup type | Implemented | `agent/micro/models.py`, `agent/micro/setups.py`, `agent/config.py`, `agent/signals/technical.py`, `cicd/env/prod.json`, `cicd/cdk/stacks/agent_runtime_stack.py`, tests | Tests cover continuation bracket, timeout, two-candle continuation confirmation, and exceptional first-candle continuation. CDK synth exposes confirmation knobs. User VS Code verification pending. | Continuation, VWAP momentum, and opening-range setups use brackets that match their observed movement profile; ordinary continuation entries wait for second-candle confirmation while rare exceptional volume moves can still enter quickly. |
...
What Changed: Added candle-direction features and a confirmation gate for `micro_volume_continuation`. Normal continuation now requires the latest and previous 1-minute candles to confirm in the trade direction. An exceptional first-candle entry remains allowed when relative volume is at least 8x and VWAP extension is controlled.
