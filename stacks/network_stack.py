...
- NAT Gateway and Elastic IP (reference only)

No new resources are created - all come from env/dev.json or env/prod.json
"""
...
class NetworkStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, config: dict, **kwargs) -> None:
        ...
        self.elastic_ip_address = STATIC_IP
        self.elastic_ip_allocation_id = NAT_EIP_ALLOCATION_ID
        ...
        CfnOutput(self, "ElasticIpAddress", 
            value=STATIC_IP, 
            export_name=f"{construct_id}:ElasticIpAddress"
        )
        ...
        print(f"🌐 VPC: {VPC_ID}")
        print(f"🔒 Private Subnet: {PRIVATE_SUBNET_ID}")
        print(f"🌍 NAT Gateway IP: {STATIC_IP}")
        print(f"🖥️  NAT Gateway ID: {NAT_GATEWAY_ID}")
        print("="*60 + "\n")